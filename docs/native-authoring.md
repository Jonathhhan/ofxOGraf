...
All updates are validated by the C++ registry even if an HTML controller has already validated them. JavaScript is an operator interface, not the authority for renderer state. The runtime enforces the serialized type, numeric `min`/`max`, enum `options`, and normalized color range before committing a merge patch. Rejections use a stable JSON-style path such as `/motion-entry-travel` and preserve the previous valid template data.
### Portable motion controls

The packaged `NativeLowerThird` demonstrates functional Content, Style, and Motion controls. Its `alignment` enum changes bitmap-text positioning consistently in native and WASM rendering. The portable `Motion` group includes position, text inset, entry/exit vectors, travel, durations, Bezier influence, scale, rotation, fade switches, and per-direction easing presets (`influence`, `linear`, `smooth`, `back`). The `back` preset exposes separate entrance overshoot and exit anticipation strength. Scale and rotation are evaluated around the moving panel centre, so they compose predictably with directional travel. These are ordinary `TemplateDefinition` controls, not browser-only settings. `EssentialControls.js` discovers them from `template-definition.json` and sends normal `updateAction()` patches; the same ids are evaluated by the native and WASM `CodeTemplate`.
